...
class SpecialGlobalUsage extends SpecialPage {
		...
		foreach ( $query->getSingleImageResult() as $wiki => $result ) {
			$wgOut->addHtml(
					'<h2>' . wfMsgExt(
						...
						$targetName, self::formatInterwikiName( $result[0]['interwiki'] ) )
					. "</h2><ul>\n" );
			foreach ( $result as $item ) {
				$wgOut->addWikiText( "* " . self::formatItem( $item ) );
			}
			$wgOut->addHtml( "</ul>\n" );
		}
		$wgOut->addHtml( '</div>' );
	...
	public static function formatItem( $item ) {
		...
		//		str_replace( '_', ' ', $page ) );
		$link = "[[:" . $item['interwiki'] . ":" . $page . "|" . $page . "]]";
		// Return only the title if no link can be constructed
		return $link === false ? $page : $link;
	}

	/**
	 * Helper to linkify an interwiki name
	 */
	public static function formatInterwikiName($name) {
		return "[[:" . $name . ":|" . $name . "]]";
	}
